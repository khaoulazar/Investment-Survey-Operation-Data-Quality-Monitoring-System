...
Spécification technique de l'environnement (SQLite + Python + Power BI), effectifs réels par région intégrés (tableau EIAP), paramètres de simulation actés (10–15% anomalies, 10% non-réponse), structure de dossiers du repo confirmée (`data/`, `scripts/`), script `generate_data.py` fonctionnel et validé (0 incohérence enquêteur/région).
